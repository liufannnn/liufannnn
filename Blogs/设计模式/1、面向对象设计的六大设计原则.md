...
// 可以看到，正方形类继承了长方形类以后，为了保证边长永远是相等的，特意在两个 set 方法里面强制将宽和高都设置为传入的值，也就是重写了父类 Rectangle 的两个 set 方法。
//
// 但是里氏替换原则里规定，子类不能重写父类的方法，所以上面的设计是违反该原则的。
//
...
rect.width = 10
rect.height = 30
let rectaArea = calculateArea(rect: rect) // 输出 300

// 现在我们使用 Rectange 的子类 Square 的对象替换原来的 Rectange 对象

let square = Square() as Rectangle
square.width = 10
square.height = 30
let squareArea = calculateArea(rect: square) // 输出 900

// 结果不一致，再次说明了上述设计不符合里氏替换原则，因为子类的对象 square 替换父类的对象 rect 以后，程序执行的结果变了。
```
...
rect.width = 10
rect.height = 30
let rectArea = calculateArea(rect: rect) // 输出 300
...
// square.width = 30
// square.height = 30
let squareArea = calculateArea(square: square) // 输出 900
```

我们可以看到，`Rectange` 和 `Square` 类都以自己的方式实现了父类 `Quadrangle` 的公共方法。而且由于 `Square` 的特殊性，它也声明了自己独有的成员变量 `sideLength` 以及其对应的公共方法。
